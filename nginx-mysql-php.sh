...
#check github config
  ...
  cd downloads
  git clone https://github.com/lauweiwei/Nginx-MySQL-PHPfpm-CentOS7.git
  cd 

  nginx1=$(md5sum /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/default1.conf | cut -c1-33)
  nginx2=$(md5sum /etc/nginx/conf.d/default1.conf | cut -c1-33)
  if [ "$nginx1" == "$nginx2" ]; then
    echo "Files are equal"
  else
    echo "Files are not equal"
    mv /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/default1.conf /etc/nginx/conf.d/default1.conf
    echo "File updated!"
  fi

  php1=$(md5sum /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/index.php | cut -c1-33)
  php2=$(md5sum /var/www/php1/index.php | cut -c1-33)
  if [ "$php1" == "$php2" ]; then
    echo "Files are equal"
  else
    echo "Files are not equal"
    mv /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/index.php /var/www/php1/index.php
    echo "File updated!"
  fi

  mysql1=$(md5sum /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/my.cnf | cut -c1-33)
  mysql2=$(md5sum /etc/my.cnf| cut -c1-33)
  if [ "$mysql1" == "$mysql2" ]; then
    echo "Files are equal"
  else
    echo "Files are not equal"
    mv /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/my.cnf /etc/my.cnf
    echo "File updated!"
  fi

  nginx3=$(md5sum /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/nginx.conf | cut -c1-33)
  nginx4=$(md5sum /etc/nginx/nginx.conf | cut -c1-33)
  if [ "$nginx3" == "$nginx4" ]; then
    echo "Files are equal"
  else
    echo "Files are not equal"
    mv /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/nginx.conf /etc/nginx/nginx.conf
    echo "File updated!"
  fi

  php3=$(md5sum /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/php.ini | cut -c1-33)
  php4=$(md5sum /etc/opt/remi/php72/php.ini | cut -c1-33)
  if [ "$php3" == "$php4" ]; then
    echo "Files are equal"
  else
    echo "Files are not equal"
    mv /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/php.ini /etc/opt/remi/php72/php.ini
    echo "File updated!"
  fi

  php5=$(md5sum /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/www.conf | cut -c1-33)
  php6=$(md5sum /etc/opt/remi/php72/php-fpm.d/www.conf | cut -c1-33)
  if [ "$php5" == "$php6" ]; then
    echo "Files are equal"
  else
    echo "Files are not equal"
    mv /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/www.conf /etc/opt/remi/php72/php-fpm.d/www.conf
    echo "File updated!"
  fi

  php7=$(md5sum /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/php-fpm.conf | cut -c1-33)
  php8=$(md5sum /etc/opt/remi/php72/php-fpm.conf | cut -c1-33)
  if [ "$php7" == "$php8" ]; then
    echo "Files are equal"
  else
    echo "Files are not equal"
    mv /root/downloads/Nginx-MySQL-PHPfpm-CentOS7/php-fpm.conf /etc/opt/remi/php72/php-fpm.conf
    echo "File updated!"
  fi
